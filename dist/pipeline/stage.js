 ...
 */
export async function executeStage(stage, initialMessages, ctx) {
    ...
    while (totalAttempts < stageConfig.maxTotalAttempts) {
        ...
        let modelAttempts = 0;
        while (modelAttempts < stageConfig.maxRetriesPerModel &&
            totalAttempts < stageConfig.maxTotalAttempts) {
            totalAttempts++;
            modelAttempts++;
            const temperature = getRepairTemperature(stageConfig.baseTemperature, modelAttempts);
            if (verbose) {
                console.error(`  [Miranda] ${stage} attempt ${totalAttempts} (model: ${model.label}, temp: ${temperature})`);
            }
            try {
                ...
                const usage = resolveTokenUsage(response.usage, promptText, response.content);
                ...
                if (verbose) {
                    console.error(`  [Miranda] Validation failed: ${(validation.errors ?? []).join(", ")}`);
                }
                currentMessages = buildRepairMessages(stage, modelAttempts, currentMessages, response.content, validation.errors ?? ["Unknown validation error"]);
...
function buildSuccessResult(stage, finalOutput, parsedData, attempts, modelUsed) {
    return {
        stage,
        success: true,
        finalOutput,
...
function buildFailureResult(stage, lastOutput, lastData, attempts) {
